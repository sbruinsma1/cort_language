...
switch(what)
    case 'SUIT:run_all'                      % STEP 9.1-8.5
        ...
        for s=1:length(subj_names),
            ...
            run_suit('SUIT:reslice',s,'contrast', space_label);
            run_suit('SUIT:plot_contrast',s);
        end
    case 'SUIT:isolate_segment'              % STEP 9.2:Segment cerebellum into grey and white matter
        ...
        SUIT_SUBJ_DIR = fullfile(SUIT_DIR, subj_names{sn});dircheck(SUIT_SUBJ_DIR);
    ...
    case 'SUIT:correct_cereb_mask'           % STEP 9.4:
        ...
        cortexGrey= fullfile(SUIT_DIR, subj_names{sn}, sprintf('c7%s_%s_T1w.nii',subj_names{sn}, space_label)); % cortex grey matter mask 
        cerebGrey = fullfile(SUIT_DIR, subj_names{sn}, sprintf('c1%s_%s_T1w.nii', subj_names{sn}, space_label)); % cerebellum grey matter mask
        bufferVox = fullfile(SUIT_DIR, subj_names{sn},'buffer_voxels.nii');
        ...
        cerebGrey2 = fullfile(SUIT_DIR, subj_names{sn},'cereb_prob_corr_grey.nii');
        cortexGrey2= fullfile(SUIT_DIR, subj_names{sn},'cortical_mask_grey_corr.nii');
    ...
    case 'SUIT:normalise_dartel'             % STEP 9.5: Normalise the cerebellum into the SUIT template.
        ...
        cd(fullfile(SUIT_DIR, subj_names{sn}));
    ...
    case 'SUIT:reslice'                      % STEP 9.8: Reslice the contrast images from first-level GLM
        ...
        switch type
            case 'contrast'
                GLM_SUBJ_DIR = fullfile(GLM_DIR, subj_names{sn});
                SUIT_SUBJ_DIR=fullfile(SUIT_DIR,subj_names{sn});
        ...
        movefile(source,SUIT_SUBJ_DIR);
        fprintf('%s have been resliced into suit space for %s \n\n',type,subj_names{sn})
    case 'SUIT:plot_contrast'
        sn=varargin{1}; % subjNum
         
        % map volume to surface and plot
        source=dir(fullfile(SUIT_DIR,subj_names{sn},'*wd*'));
        
        for m=1:length(source),

            C=suit_map2surf(fullfile(SUIT_DIR,subj_names{sn}, source(m).name),'stats','nanmean');
            figure(m);
            suit_plotflatmap(C, 'cscale',[-2 2])
            title(strrep(source(m).name, '.nii', '')); 
        end
end
