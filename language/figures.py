...
def fig1():
    ...
    vis_task.plot_slope(df, y='cloze', plot_type='bar', ax=ax)
    ...
    vis_task.plot_slope(df, y='CoRT', plot_type='bar', ax=ax)
...
def figS1():
    ...
    ax = fig.add_subplot(gs[0,1])
    vis_task.plot_rt(df, plot_type='line', hue='group', ax=ax)
    ax.text(x_pos, y_pos, 'B', transform=ax.transAxes, fontsize=labelsize, verticalalignment='top')
...
def figS2():
    ...
    ax = fig.add_subplot(gs[0,0])
    ax = vis_task.scatterplot_rating(df, x='cloze', hue='group')
    plt.xticks([0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1])
    ax.text(x_pos, y_pos, 'A', transform=ax.transAxes, fontsize=labelsize, verticalalignment='top')

    ax = fig.add_subplot(gs[0,1])
    ax = vis_task.scatterplot_rating(df, x='CoRT', hue='group')
